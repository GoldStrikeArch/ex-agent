defmodule AgentCore.ToolExecutorTest do
  ...
  test "read_file returns content and emits tool events", %{workspace: workspace} do
    ...
    assert {:ok, result} =
             AgentCore.run_tool("read_file", %{"path" => "lib/example.ex"},
    ...
    assert_receive {:agent_core_event, {:tool_output, "tool-read", output}}
    assert output =~ "defmodule Example"
    assert_receive {:agent_core_event, {:tool_finished, "tool-read", :ok, summary}}
    assert summary =~ "read lib/example.ex"
  end

  test "list_files returns sorted direct children", %{workspace: workspace} do
    assert {:ok, result} =
             AgentCore.run_tool("list_files", %{path: "."},
               workspace_root: workspace,
  ...
  test "grep returns ripgrep matches", %{workspace: workspace} do
    assert {:ok, result} =
             AgentCore.run_tool("grep", %{pattern: "hello", path: "lib"},
               workspace_root: workspace,
  ...
  test "unknown tools return structured errors and emit failure events", %{workspace: workspace} do
    ...
    assert {:error, {:unknown_tool, "missing"}} =
             AgentCore.run_tool("missing", %{},
               workspace_root: workspace,
               tool_call_id: "tool-missing"
             )

    assert_receive {:agent_core_event, {:error, :tool, {:unknown_tool, "missing"}}}
    assert_receive {:agent_core_event, {:tool_finished, "tool-missing", :error, summary}}
