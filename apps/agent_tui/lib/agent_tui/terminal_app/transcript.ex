defmodule AgentTui.TerminalApp.Transcript do
  @moduledoc """
  Maintains the visible transcript as completed lines plus the active line.

  `AgentTui.TextRenderer` remains the canonical event-to-text renderer. This
  ...
  """
  @spec append_event(t(), AgentCore.Event.t()) :: t()
  def append_event(%__MODULE__{} = transcript, event) do
    event
    |> AgentTui.TextRenderer.render()
    |> IO.iodata_to_binary()
