defmodule AgentTui.TerminalApp.Root do
  @moduledoc """
  ...
  alias AgentTui.TerminalApp.CommandMenu
  alias AgentTui.TerminalApp.Prompt
  alias AgentTui.TerminalApp.Status
  alias AgentTui.TerminalApp.Transcript
  alias TermUI.Event
  ...
  def init(opts) do
    ...
    %__MODULE__{
      ...
      status: Status.new(),
      task_supervisor: Keyword.get(opts, :task_supervisor, AgentTui.TaskSupervisor),
  ...
  def update({:set_session, session}, state) when is_pid(session) do
  ...
  def update({:agent_core_event, event}, state) do
    state =
      %{
        state
  ...
  defp submit_prompt(%{session: session} = state, prompt) when is_pid(session) do
    prompt_ref = make_ref()
    runtime = self()

    with {:ok, _pid} <-
           start_prompt_task(state.task_supervisor, fn ->
             result = AgentCore.send_message(session, prompt)
  ...
  defp submit_prompt(state, _prompt) do
    %{state | notice: "submit failed: session is not running"}
  end
