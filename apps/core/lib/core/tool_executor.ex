defmodule Core.ToolExecutor do
  @moduledoc """
  ...
  """
  @spec run(String.t(), map(), keyword()) :: {:ok, Core.Tool.result()} | {:error, term()}
  ...
  def run(name, args, opts) when is_binary(name) and is_map(args) do
    ...
    with {:ok, tool} <- Core.ToolRegistry.fetch(name, tools) do
      ...
      {:error, reason} = error ->
        emit(Core.Event.error(:tool, reason))
        emit(Core.Event.tool_finished(tool_call_id, :error, summarize_error(reason)))
        error
    end
  end
  ...
  defp finish({:ok, result}, tool_call_id, _name) when is_map(result) do
    ...
    emit(Core.Event.tool_finished(tool_call_id, :ok, Map.get(result, :summary, "completed")))
  ...
  defp emit_output(output, tool_call_id) when is_binary(output) do
    emit(Core.Event.tool_output(tool_call_id, truncate(output)))
  end
